...
namespace ProjectMayhem.Controllers
{
    public class TeamController : Controller
    {
        ...
        public async Task<ActionResult> Members(MembersViewModel model)
        {
            ...
            var NewLeaderObj = await UserManager.FindByIdAsync(model.NewLeadId);
            ...
            else if (ReassignedMember == null)
                ModelState.AddModelError("", "Team Member with that name does not exist");
            else if(NewLeaderObj == null)
                ModelState.AddModelError("", "New Team Leader with that name does not exist");
